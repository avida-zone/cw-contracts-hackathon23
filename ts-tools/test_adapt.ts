...
(async function adapt_tf() {
  ...
  nonce = await qs.queryWasm(rg1_transform_address.default, {
    proof_nonce: { address: wallet },
  });
  console.log("proof nonce for unadapt: ", nonce);
  proof = await generateProof(user.address, wallet, nonce);
  ...
  proxy_msg: ProxyT.CosmosMsgForEmpty = {
    wasm: {
      execute: {
        contract_addr: rg1_transform_address.default,
        funds: [{ denom: "rginj", amount: "20" }],
