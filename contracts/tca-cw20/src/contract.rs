...
/// Verifies the proof on the anoncreds verifier contract
/// Currently we are using query,
/// but it might be a service that users have to pay for.
pub fn verify_vc_proof(
    ...
    let verification_msg = vc_verifier::msg::ExecuteMsg::Verify {
        proof,
        proof_req_nonce,
        wallet_addr,
    };
    let result = deps
        .querier
        .query_wasm_smart(VERIFIER.load(deps.storage)?, &verification_msg)
        .map_err(|e| ContractError::VCVError(e.into()))?;
    if result {
        Ok(())
    } else {
        Err(ContractError::ProofInvalid {})
    }
}
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::Transfer {
            ...
            proof,
            proof_req_nonce,
        } => execute_transfer(deps, env, info, recipient, amount, proof, proof_req_nonce),
        ExecuteMsg::Burn {
            amount,
            proof,
            proof_req_nonce,
        } => execute_burn(deps, env, info, amount, proof, proof_req_nonce),
        ExecuteMsg::Send {
            contract,
            amount,
            msg,
...
pub fn execute_transfer(
    ...
    verify_vc_proof(deps.as_ref(), info.sender.clone(), proof, proof_req_nonce)?;
